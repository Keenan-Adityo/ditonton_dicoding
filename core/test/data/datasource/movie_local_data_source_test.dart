...
void main() {
  ...
  group('cache movies', () {
    test('should call database helper to save data', () async {
      // arrange
      when(mockDatabaseHelper.clearMovieCache('now_playing'))
          .thenAnswer((_) async => 1);
      // act
      await dataSource.cacheNowPlayingMovies([testMovieCache]);
      // assert
      verify(mockDatabaseHelper.clearMovieCache('now_playing'));
      verify(mockDatabaseHelper
          .insertMovieCacheTransaction([testMovieCache], 'now_playing'));
    });

    test('should return list of movies from db when data exist', () async {
      // arrange
      when(mockDatabaseHelper.getMovieCache('now_playing'))
